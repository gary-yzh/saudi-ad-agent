...
| 7 | 1:50–2:05 | Point at one shot's Apply box (the textarea under each image) | **(0:15)** "Each shot has a little Apply box below it — you can type something like 'darker background' and the agent regenerates just that one shot. And if Doubao ever rejects an image as too sensitive, it auto-rewrites the prompt up to four times until something passes. Users almost never see a 'rejected' error." |
